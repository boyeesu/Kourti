...
import { ArrowLeft, Send, MessageCircle, Paperclip, X, FileText, Image, File, Loader2, Download } from 'lucide-react';
...
export function ChatWindow({ conversationId, onClose, recipientName }: ChatWindowProps) {
  ...
  const renderFileAttachment = (msg: Message) => {
    ...
    const handleDownload = async () => {
      if (!metadata.file_url) return;
      window.open(metadata.file_url, '_blank');
    };

    return (
      <div className="mt-1">
        {isImage && metadata.file_url ? (
