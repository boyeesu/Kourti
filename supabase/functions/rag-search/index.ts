...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const documentIds = Array.from(
      new Set(searchResults.map((r: any) => r.document_id).filter(Boolean))
    );
    const contractIds = Array.from(
      new Set(searchResults.map((r: any) => r.contract_id).filter(Boolean))
    );
    ...
    const contractNameMap = new Map(
      (contractsResponse.data || []).map((c: any) => [c.id, c.title])
    );
