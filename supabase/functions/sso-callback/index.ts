...
serve(async (req) => {
  ...
  try {
    ...
    const { data: configs } = await supabase
      ...
      .select(
        'id, provider, organization_id, client_id, client_secret, tenant_id, is_enabled, domain'
      )
    ...
    if (!finalConfig) {
      console.error('No SSO config found for email domain', {
        email,
        emailDomain,
        provider: payload.provider,
      });
      throw new Error(
        `Your email domain (${emailDomain}) is not configured for SSO with this provider`
      );
    }
    ...
    const { data: existingProfile } = (await supabase
      ...
      .maybeSingle()) as {
      data: { user_id: string; organization_id: string | null; status: string } | null;
      error: any;
    };
    ...
    if (
      ...
      existingProfile.organization_id !== config.organization_id
    ) {
    ...
    if (fallbackUrl) {
      const location = buildRedirectWithError(fallbackUrl, 'authentication_failed');
      return createEmptyResponse({
        status: 302,
        cors: corsOptions,
        headers: { Location: location },
