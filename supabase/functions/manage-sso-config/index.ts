...
const ALLOWED_REDIRECT_PATHS = ['/auth/callback', '/sso/callback', '/auth'];
...
function validateRedirectUri(uri?: string | null) {
  ...
  try {
    ...
    // Check path
    const isValidPath = ALLOWED_REDIRECT_PATHS.some(
      (validPath) => parsed.pathname === validPath || parsed.pathname.startsWith(validPath + '/')
    );
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const isSuperadmin = roleAssignments?.some(
      (r: { role_name: string }) => r.role_name === 'superadmin'
    );
    ...
    switch (request.action) {
      case 'create': {
        ...
        if (error) {
          ...
          throw new HttpError(
            `Failed to create SSO config: ${error.message}`,
            400,
            'SSO_CREATE_FAILED'
          );
        }
      ...
      case 'update': {
        ...
        if (existing.organization_id !== profile.organization_id) {
          throw new HttpError(
            'SSO configuration not found in your organization',
            404,
            'SSO_NOT_FOUND'
          );
        }
        ...
        if (error) {
          ...
          throw new HttpError(
            `Failed to update SSO config: ${error.message}`,
            400,
            'SSO_UPDATE_FAILED'
          );
        }
      ...
      case 'rotate': {
        ...
        if (!payload.id || !payload.clientSecret) {
          throw new HttpError(
            'id and clientSecret are required for rotation',
            400,
            'INVALID_INPUT'
          );
        }
        ...
        if (error) {
          ...
          throw new HttpError(
            `Failed to rotate SSO secret: ${error.message}`,
            400,
            'SSO_ROTATE_FAILED'
          );
        }
      ...
      case 'delete': {
        ...
        if (error) {
          ...
          throw new HttpError(
            `Failed to delete SSO config: ${error.message}`,
            400,
            'SSO_DELETE_FAILED'
          );
        }
      ...
      case 'test': {
        ...
        if (
          ...
          (!existing.tenant_id || existing.tenant_id.trim() === '')
        ) {
